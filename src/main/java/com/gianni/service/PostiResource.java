...
public class PostiResource {
	...
	@ApiOperation(value = "Restituisce tutti i posti disponibili", response = Posti.class, responseContainer = "List")
	...
	@Path("/{tipologia}")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "Restituisce una lista di posti filtrati per tipologia", response = Posti.class, responseContainer = "List")
	...
	public Response creaPosto(@ApiParam(value = "Posto da creare") Posti request) {
		boolean esito = DAOPosto.inserisciNuovoPosto(request);
	...
	public Response uploadFoto(@ApiParam(value = "id del posto") @PathParam("id") int id,
		...
		if (null != foto) {
			...
			esito = DAOPosto.aggiornaFoto(id, foto);
			response.setEsito(esito);
